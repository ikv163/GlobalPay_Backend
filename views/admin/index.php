            ...
            <div class="pad-all">
                <div class="media pad-ver">
                    <div class="media-left">
                        <img alt="Profile Picture" class="img-md img-circle" src="<?= empty($model->photo) ? Admin::getProfilePhotos(true) : Html::encode($model->photo); ?>">
                    </div>
                    <div class="media-body" style="padding-top: 10px;">
                        <div class="box-inline">
                            <span class="text-lg text-semibold text-main"><?= isset($model->username) ? Html::encode($model->username) : '-'; ?></span>
                ...
                <div class="pad-btm bord-bt text-sm">
                	<div class="row pad-top-5">
    					...
    					<div class="col-xs-10"><?= empty($model->admin->name) ? Yii::t('app', 'not_set') : Html::encode($model->admin->name); ?></div>
                	</div>
                	<div class="row pad-top-5">
    					<div class="col-xs-2 text-right text-main text-semibold"><i class="pli-smartphone-3 icon-lg icon-fw"></i></div>
    					<div class="col-xs-10"><?= empty($model->admin->mobile) ? Yii::t('app', 'not_set') : Html::encode($model->admin->mobile); ?></div>
                	</div>
                	<div class="row pad-top-5">
    					<div class="col-xs-2 text-right text-main text-semibold"><i class="pli-email icon-lg icon-fw"></i></div>
    					<div class="col-xs-10"><?= empty($model->admin->email) ? Yii::t('app', 'not_set') : Html::encode($model->admin->email); ?></div>
                	</div>
				</div>
